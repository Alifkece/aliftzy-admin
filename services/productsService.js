...
/** Upload gambar produk ke Firebase Storage, mengembalikan download URL
 *  yang langsung kompatibel dengan `<img src>` yang dipakai Store. */
export async function uploadProductImage(id, dataUrl) {
  const path = `products/${id || genId("prod")}-${Date.now()}.jpg`;
